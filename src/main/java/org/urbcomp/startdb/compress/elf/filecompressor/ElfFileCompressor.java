...
import org.urbcomp.startdb.compress.elf.utils.WriteByte;
...
//            bos.write(result, 0, sizeofcompressor);
        ...
        while ((vs = fileReader.nextBlock()) != null) {
            ...
            int sizeofcompressor = compressor.getSize() / 8 + 12;// todo:+12?
//            byte[] sizeOfBlock = intToTwoBytes(sizeofcompressor);
            byte[] sizeOfBlock = ByteToInt.intToBytes2(sizeofcompressor);
            //CountOfBlock 记录有多少个块
            if (sizeList.get(3) != (byte) 0xff) {
                sizeList.set(3, (byte) (sizeList.get(3) + 1));

            }
            else if (sizeList.get(3) == (byte) 0xff && sizeList.get(2) != (byte) 0xff){
                sizeList.set(2, (byte) (sizeList.get(2) + 1));
                sizeList.set(3, (byte) 0x00);
            }
            else if (sizeList.get(2) == (byte) 0xff && sizeList.get(1) != (byte) 0xff){
                sizeList.set(1, (byte) (sizeList.get(1) + 1));
                sizeList.set(2, (byte) 0x00);
            }
            else {
                sizeList.set(0, (byte) (sizeList.get(0) + 1));
                sizeList.set(1, (byte) 0x00);
...
//            }
            ...
            sizeList.add(sizeOfBlock[0]);
            sizeList.add(sizeOfBlock[1]);
            sizeList.add(sizeOfBlock[2]);
            sizeList.add(sizeOfBlock[3]);
...
//        }
        WriteByte.insert2(filePath, "D:/11.elf", blocks);
        filePath = "D:/11.elf";
...
//        WriteByteToCSV.writeByteToCSV(filePath,1);
        WriteByte.insert(filePath, "D:/1.elf", 1);
        fos.close();
