...
public class InputBitStream implements BooleanIterator, Flushable, Closeable {
    /* Precomputed tables: the i-th entry decodes the stream fragment of 16 bits given by the binary representation of i.
...
//		}
    ...
    public static boolean checkLength(final String resource, final int[] array, final String resouceFullPath) {
        final DataInputStream dis = new DataInputStream(Objects.requireNonNull(InputBitStream.class.getResourceAsStream(resouceFullPath)));
...
//		return fill += 16;
    ...
    public void position(final long position) throws IOException {
        ...
        if (DEBUG)
            System.err.println(this + ".position(" + position + "); curr: " + this.position + " delta: " + delta + " pos: " + pos + " avail: " + avail);
