 ...
 * the first bit of the stream is bit 7 of the first byte, the eighth bit
 ...
 * which is identical to the one produced by a {@link DataOutput}.
 *
...
public class OutputBitStream implements Flushable, Closeable {
    ...
    static {
         ...
         * to work around the limit on static initializer code. */
        try {
            org.urbcomp.startdb.serf.utils.InputBitStream.fillArrayFromResource("gamma.out.12", GAMMA);
            org.urbcomp.startdb.serf.utils.InputBitStream.fillArrayFromResource("delta.out.12", DELTA);
            org.urbcomp.startdb.serf.utils.InputBitStream.fillArrayFromResource("zeta3.out.12", ZETA_3);
            org.urbcomp.startdb.serf.utils.InputBitStream.fillArrayFromResource("shiftedgamma.out.12", SHIFTED_GAMMA);
    ...
    public OutputBitStream(final OutputStream os, final int bufSize, final boolean testForPosition) {
        ...
        if (os instanceof RepositionableStream) {
            ...
            } catch (final IllegalAccessException | IllegalArgumentException | NoSuchMethodException |
                    ClassCastException |
     ...
     */
    public void copyFrom(final org.urbcomp.startdb.serf.utils.InputBitStream ibs, long length) throws IOException {
